...
function send_rfq_fn()
{
    if (isset($_POST['quantity'])) {
        ...
        if ($post_id) {
            ...
            $output = wp_upload_bits($my_custom_filename, null, file_get_contents($_FILES['attachment']['tmp_name']));
            ...
            add_post_meta($post_id, 'attachment', $output['url']);
            add_post_meta($post_id, 'attachment_name', $_FILES['attachment']['name']);
...
function submit_proposal_fn()
{
    ...
    if ($post_id) {
        ...
        update_post_meta($post_id, 'attachment', $output['url']);
        update_post_meta($post_id, 'attachment_name', $_FILES['attachment']['name']);
...
function remove_proposal_fn()
{
    ...
    if ($proposal && $proposal->post_author == get_current_user_id()) {
        ...
        delete_post_meta($post_id, 'attachment_name');
...
?>
    ...
    <table class="form-table">
        <tr>
            ...
            <td><input type="checkbox" name="is_paid" value="1" <?= (get_the_author_meta('is_paid', $user->ID) == 1) ? 'checked="checked"' : ''; ?> class="regular-text" /></td>
        </tr>
