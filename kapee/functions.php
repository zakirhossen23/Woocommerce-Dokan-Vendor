		...
		jQuery(document).ready(function($) {
			$('.send-quote-form').on('submit', function(e) {
				...
				jQuery.ajax({
					...
					data: { action: 'send_quotes_to_seller', val:val },
		    ...
		    while ( $the_query->have_posts() ) {
		        ...
		        ?>
				  	<tr>
                        ...
                        <a href="mailto: <?php echo get_the_author_meta('user_email', $author_id); ?>"><?php echo get_the_author_meta('display_name', $author_id); ?></a></td>
							...
							jQuery(document).ready(function($) {
								...
								$('.send-proposal-form-<?php echo $post->ID;?>').on('submit', function(e) {
									...
									jQuery.ajax({
										...
										context: this,
										url: wc_add_to_cart_params.ajax_url,
										data: { action: 'send_proposal_to_buyer', val:val },
										success: function(response) {
											$('.message').html('<p class="alert-success">Successfully Sent</p>');
											location.reload();
										},
