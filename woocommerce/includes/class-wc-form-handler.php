...
class WC_Form_Handler {
	...
	public static function save_account_details() {
		...
		if ($role_type === "hospital"){
			$account_hospital_name   = ! empty( $_POST['account_hospital_name'] ) ? wc_clean( wp_unslash( $_POST['account_hospital_name'] ) ) : '';
			$account_person_incharge   = ! empty( $_POST['account_person_incharge'] ) ? wc_clean( wp_unslash( $_POST['account_person_incharge'] ) ) : '';
			$account_hospital_phone   = ! empty( $_POST['account_hospital_phone'] ) ? wc_clean( wp_unslash( $_POST['account_hospital_phone'] ) ) : '';
			$account_hospital_address   = ! empty( $_POST['account_hospital_address'] ) ? wc_clean( wp_unslash( $_POST['account_hospital_address'] ) ) : '';
			$user_details = array(
                'hospital_name'     => sanitize_text_field( wp_unslash( $account_hospital_name ) ),  
                'person_incharge'     => sanitize_text_field( wp_unslash( $account_person_incharge ) ),
                'hospital_phone'     => sanitize_text_field( wp_unslash( $account_hospital_phone ) ),
                'hospital_address'     => sanitize_text_field( wp_unslash( $account_hospital_address ) )
            );
		...
		if ($role_type === "seller"){
			...
			$user_details = array(
                'company_name'     => sanitize_text_field( wp_unslash( $account_company_name ) ),  
                'contact_person'     => sanitize_text_field( wp_unslash( $account_contact_person ) ),
                'seller_phone'     => sanitize_text_field( wp_unslash( $account_seller_phone ) ),
                'seller_address'     => sanitize_text_field( wp_unslash( $account_seller_address ) )
            );
		...
		if ($role_type === "customer"){
			...
			$account_customer_phone   = ! empty( $_POST['account_customer_phone'] ) ? wc_clean( wp_unslash( $_POST['account_customer_phone'] ) ) : '';
			$account_customer_address   = ! empty( $_POST['account_customer_address'] ) ? wc_clean( wp_unslash( $_POST['account_customer_address'] ) ) : '';
	
			$user_details = array(
                'customer_name'     => sanitize_text_field( wp_unslash( $account_customer_name ) ),  
                'customer_phone'     => sanitize_text_field( wp_unslash( $account_customer_phone ) ),
                'customer_address'     => sanitize_text_field( wp_unslash( $account_customer_address ) )
             );
	...
	public static function process_login() {
		...
		if ( isset( $_POST['login'], $_POST['username'], $_POST['password'] ) && wp_verify_nonce( $nonce_value, 'woocommerce-login' ) ) {
			...
			try {
				...
				$user_data = (object)get_user_by( is_email( $creds['user_login'] ) ? 'email' : 'login', $creds['user_login'] );
	
				if (isset($user_data->user_status) ){
					if (strcmp(strval($user_data->user_status), "1") === 0){
						throw new Exception( '<strong>' . __( 'Error: ', 'woocommerce' ) . $creds['user_login'] .'</strong> ' . '<span>'.' is pending for approval.' . '</span>' );
					}
					if ( strcmp(strval($user_data->user_status), "2") === 0){
						throw new Exception( '<strong>' . __( 'Error:', 'woocommerce' ) . $creds['user_login'] .'</strong> ' . __( ' is rejected.', 'woocommerce' ) );
					}
				}
	...
	public static function process_registration() {
		...
		if ( isset( $_POST['register'], $_POST['email'] ) && wp_verify_nonce( $nonce_value, 'woocommerce-register' ) ) {
			...
			try {
				...
				wc_add_notice( __( 'Thank you for your registration with Tekoa. We will contact you soon.', 'woocommerce' ) );
